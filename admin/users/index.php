<?php
// ============================================
// ADMIN: Users Management - Subhan Printers
// ============================================
...
// Get all users
$users = $userModel->getAll([], 200, 0);

// Handle toggle role
if (isset($_GET['role']) && is_numeric($_GET['id'])) {
    $id = (int)$_GET['id'];
    $role = $_GET['role'];
    if (in_array($role, ['admin', 'staff', 'customer'])) {
        $userModel->updateRole($id, $role);
        header('Location: /SP/admin/users/index.php?msg=updated');
        exit;
    }
}
...
if (isset($_GET['toggle']) && is_numeric($_GET['toggle'])) {
    $userModel->toggleActive($_GET['toggle']);
    header('Location: /SP/admin/users/index.php?msg=updated');
    exit;
}
...
if (isset($_GET['delete']) && is_numeric($_GET['delete'])) {
    ...
    header('Location: /SP/admin/users/index.php?msg=deleted');
    exit;
}
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .user-name { color: #e8e8f0; font-weight: 600; }
        .user-email { color: #8888aa; font-size: 0.8rem; }
        ...
        .action-btn.role:hover { background: rgba(139,92,246,0.25); }
        .action-btn.toggle {
...
<aside class="sidebar">
    ...
    <ul class="sidebar-menu">
        ...
        <li><a href="/SP/admin/users/" class="active"><i class="fas fa-users"></i><span>Users</span></a></li>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <br><span class="user-email"><?php echo htmlspecialchars($user['email'] ?? ''); ?></span>
                    </td>
                    <td><?php echo htmlspecialchars($user['phone'] ?? '—'); ?></td>
                    <td>
                        <span class="role-badge" style="background:<?php echo $roleColors[$user['role'] ?? 'customer'] ?? '#6b7280'; ?>20;color:<?php echo $roleColors[$user['role'] ?? 'customer'] ?? '#6b7280'; ?>;">
                            <?php echo ucfirst($user['role'] ?? 'customer'); ?>
                        </span>
                    </td>
                    <td>
                        <span class="status-badge <?php echo ($user['is_active'] ?? 1) ? 'status-active' : 'status-inactive'; ?>">
                            <span class="dot" style="background:<?php echo ($user['is_active'] ?? 1) ? '#22c55e' : '#ef4444'; ?>;"></span>
                            <?php echo ($user['is_active'] ?? 1) ? 'Active' : 'Inactive'; ?>
                        </span>
                    </td>
                    <td style="font-size:0.8rem;"><?php echo date('M d, Y', strtotime($user['created_at'] ?? 'now')); ?></td>
                    <td style="text-align:center;">
                        <div style="display:flex;gap:4px;flex-wrap:wrap;justify-content:center;">
                            <a href="?id=<?php echo $user['id']; ?>&role=admin" class="action-btn role"><i class="fas fa-crown"></i></a>
                            <a href="?id=<?php echo $user['id']; ?>&role=staff" class="action-btn role"><i class="fas fa-user-tie"></i></a>
                            <a href="?id=<?php echo $user['id']; ?>&role=customer" class="action-btn role"><i class="fas fa-user"></i></a>
                            <a href="?toggle=<?php echo $user['id']; ?>" class="action-btn toggle">
                                <i class="fas fa-<?php echo ($user['is_active'] ?? 1) ? 'pause' : 'play'; ?>"></i>
                            </a>
                            <button onclick="deleteUser(<?php echo $user['id']; ?>)" class="action-btn delete"><i class="fas fa-trash"></i></button>
                        </div>
                    </td>
                </tr>
                <?php endforeach; ?>
...
function deleteUser(id) {
    if (confirm('Are you sure you want to delete this user?')) {
        fetch('/SP/api/users?id=' + id, { method: 'DELETE' })
        .then(r => r.json())
        .then(d => {
