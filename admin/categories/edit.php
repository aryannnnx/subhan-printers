...
if (!$category) {
    header('Location: /SP/admin/categories/index.php');
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($name)) {
        ...
        if ($result['success']) {
            header('Location: /SP/admin/categories/index.php?msg=updated');
            exit;
        } else {
    ...
    <div class="form-card">
        <form method="POST" action="">
            ...
            <div class="form-group">
                ...
                <input type="text" name="name" placeholder="e.g., Wedding Cards" required value="<?php echo htmlspecialchars($category['name'] ?? ''); ?>">
            </div>
            ...
            <div class="form-group">
                ...
                <div class="icon-grid">
                    ...
                    <label class="icon-option <?php echo ($category['icon'] ?? '') === $icon ? 'selected' : ''; ?>">
                        <i class="fas <?php echo $icon; ?>"></i>
                        <input type="radio" name="icon" value="<?php echo $icon; ?>" <?php echo ($category['icon'] ?? '') === $icon ? 'checked' : ''; ?>>
                    </label>
                    <?php endforeach; ?>
                </div>
            ...
            <div class="form-group">
                ...
                <textarea name="description" placeholder="Category description"><?php echo htmlspecialchars($category['description'] ?? ''); ?></textarea>
            </div>

            <div class="form-group">
                <label>Display Order</label>
                <input type="number" name="display_order" placeholder="0" value="<?php echo htmlspecialchars($category['display_order'] ?? 0); ?>">
                <div class="hint">Higher number = appears later in list</div>
            </div>

            <div class="checkbox-group" style="margin-bottom:20px;">
                <input type="checkbox" name="is_active" id="is_active" <?php echo ($category['is_active'] ?? 1) ? 'checked' : ''; ?>>
