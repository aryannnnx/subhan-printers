<?php
// ============================================
// ADMIN: Categories List - Subhan Printers
// ============================================
...
if (isset($_GET['delete']) && isset($_GET['id'])) {
    ...
    $usage = $categoryModel->getUsageCount($id);
    
    if ($usage['total'] > 0) {
        // Category is in use - soft delete (deactivate)
        $result = $categoryModel->delete($id);
        if ($result['success']) {
            $msg = 'Category deactivated! (Used in ' . $usage['products'] . ' products, ' . $usage['portfolio'] . ' portfolio items)';
            header('Location: /SP/admin/categories/index.php?msg=' . urlencode($msg));
        } else {
            header('Location: /SP/admin/categories/index.php?error=Failed to deactivate category');
        }
    } else {
        // Category is not in use - hard delete
        $result = $categoryModel->deletePermanent($id);
        if ($result['success']) {
            header('Location: /SP/admin/categories/index.php?msg=Category deleted successfully!');
        } else {
            header('Location: /SP/admin/categories/index.php?error=' . urlencode($result['error'] ?? 'Failed to delete category'));
        }
    }
    exit;
}
...
<main class="main-content">
  ...
  <!-- ... your sidebar and HTML ... -->
    ...
    <div class="action-bar">
        <div>
            <span style="color:#8888aa;">Total Categories: <strong style="color:#e8e8f0;"><?php echo count($categories); ?></strong></span>
        </div>
        <a href="/SP/admin/categories/add.php" class="btn-add">
    ...
    <div class="table-wrap">
        ...
        <table>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td style="color:#555577;"><?php echo $cat['slug']; ?></td>
                    <td>
                        <?php 
                        $count = $categoryModel->countProducts($cat['id']);
                        echo $count;
                        ?>
                    </td>
                    <td>
                        <span class="badge-status <?php echo $cat['is_active'] ? 'badge-active' : 'badge-inactive'; ?>">
                    ...
                    <td style="text-align:right;">
                        <div class="action-btns" style="justify-content:flex-end;">
                            <a href="/SP/admin/categories/edit.php?id=<?php echo $cat['id']; ?>" class="btn-edit">
                                <i class="fas fa-edit"></i>
                            </a>
                            <a href="/SP/admin/categories/index.php?delete=1&id=<?php echo $cat['id']; ?>" 
                               class="btn-delete" 
        ...
        <div class="empty-state">
            ...
            <a href="/SP/admin/categories/add.php" class="btn-add" style="margin-top:16px;">
